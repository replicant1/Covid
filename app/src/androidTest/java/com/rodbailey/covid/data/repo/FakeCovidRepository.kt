...
class FakeCovidRepository() : ICovidRepository {
    ...
    companion object {
        val REGIONS = mapOf(
            ...
            Region("___", "Global") to ReportData(
                confirmed = 88L,
                deaths = 66L,
                recovered = 55L,
                active = 54L,
    ...
    override suspend fun getRegions(): List<Region> {
        Timber.d("**** Into FakeCovidRepositoy.getRegions() ***")
        if (nextOpThrowsException) {
            throw RuntimeException()
        }
        return REGIONS.keys.sortedBy { it.name }
    }
    ...
    override suspend fun getReport(isoCode: String?): ReportData {
        Timber.d("**** Into FakeCovidRepository.getReport $isoCode")
        if (nextOpThrowsException) {
            throw RuntimeException()
        }
        val safeIsoCode = isoCode  ?: "___"
        val result = REGIONS.keys.filter { it.iso3Code == safeIsoCode }
        if (result.isEmpty()) {
            throw RuntimeException("No region found with ISO code $safeIsoCode")
        }
        val matchingData = REGIONS[result[0]]
        return matchingData ?: throw RuntimeException("No region found with ISO code $safeIsoCode")
    }

    override suspend fun getRegionsByName(searchText: String): List<Region> {
        Timber.d("*** Into FakecdovidRepository.getRegionsByName $searchText")
        if (nextOpThrowsException) {
            throw RuntimeException()
        }
        return REGIONS.keys.filter { region ->
